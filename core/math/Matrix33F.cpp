...
void Matrix33F::transform1(Vector3F& b, const Vector3F & a) const
{
...
void Matrix33F::transformReversed(Vector3F& b, const Vector3F & a) const
{
	b.x = -a.x * M(0, 0) - a.y * M(1, 0) - a.z * M(2, 0);
	b.y = -a.x * M(0, 1) - a.y * M(1, 1) - a.z * M(2, 1);
	b.z = -a.x * M(0, 2) - a.y * M(1, 2) - a.z * M(2, 2);
...
void Matrix33F::transformInPlace(Vector3F& b) const
{
	const float ax = b.x;
	const float ay = b.y;
	b.x = ax * M(0, 0) + ay * M(1, 0) + b.z * M(2, 0);
	b.y = ax * M(0, 1) + ay * M(1, 1) + b.z * M(2, 1);
	b.z = ax * M(0, 2) + ay * M(1, 2) + b.z * M(2, 2);
}
