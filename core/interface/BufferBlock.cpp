...
void BufferBlock::projectImage(DisplayImage* img) const
{
	...
	const float divage = inverseAge() * 255.f;
	...
	for(int j=0;j<m_packetY;++j) {
		...
		for(int i=0;i<m_packetX;++i) {
			...
			int r = rfrag[ind] * divage + .5f; Clamp0255(r);
			int g = gfrag[ind] * divage + .5f; Clamp0255(g);
			int b = bfrag[ind] * divage + .5f; Clamp0255(b);
			int a = afrag[ind] * divage + .5f; Clamp0255(a);
...
{ 
	m_age = 0;
	m_residual = RandomFlh(10000.f, 999999.f);  
}
...
float BufferBlock::inverseAge() const
{ return (m_age < 1) ? 1.f : (1.f / (float)m_age); }
